...
# --> we need to make to factor this into our alignment in the experiments.
...
# constant_voxels = np.where(abs(np.std(scans, 0)) == 0)[0]
# print("Constant Voxels")
# print(constant_voxels)
# print(len(constant_voxels))
    #stimuli = [event.stimulus_pointers for event in block.scan_events]
    #timestamps = [event.timestamp for event in block.scan_events]
...
for subject in harry_data.keys():
    ...
    block = harry_data[subject][-1]
    sentences = block.sentences
    scans = [event.scan for event in block.scan_events]

    print(subject, block, len(sentences))
    # print(len(scans[0]))
